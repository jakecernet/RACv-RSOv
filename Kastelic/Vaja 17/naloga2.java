/* Realizirajte ustrezno ažuriranje podatkov v podatkovni zbirki po vsakem igranju. */
...
public class naloga2 {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            while (!gameOver && wrongGuesses < 6) {
                ...
                if (isWordGuessed()) {
                    ...
                    updateDatabaseAfterGame(conn, passwordId, true, totalGuesses, wrongGuesses);
                } else if (wrongGuesses >= 6) {
                    gameOver = true;
                    System.out.println("\n✗ IZGUBIL SI! Geslo je bilo: " + currentPassword);
                    System.out.println("Poskusov: " + totalGuesses + " | Napake: " + wrongGuesses);
                    updateDatabaseAfterGame(conn, passwordId, false, totalGuesses, wrongGuesses);
                }
            }
            
            // Display updated statistics
            displayPasswordStatistics(conn, passwordId);
     ...
     */
    private void updateDatabaseAfterGame(Connection conn, int id, boolean success, int totalGuesses, int wrongGuesses) throws SQLException {
        String sql = """
            UPDATE besede 
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (rowsAffected > 0) {
                ...
                if (success) {
                    System.out.println("  - Uspešen poskus evidentiran");
                }
                System.out.println("  - Zadnja uporaba: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
            }
        }
    }
    
    /**
     * Displays updated statistics for the password from the database.
     */
    private void displayPasswordStatistics(Connection conn, int id) throws SQLException {
        String sql = """
            SELECT geslo, st_uporab, uspesni_poskusi, zadnja_uporaba, ocenjena_zahtevnost,
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                System.out.println("\n=== STATISTIKA GESLA ===");
                System.out.println("Geslo: " + rs.getString("geslo"));
                System.out.println("Skupno uporab: " + rs.getInt("st_uporab"));
                System.out.println("Uspešni poskusi: " + rs.getInt("uspesni_poskusi"));
                System.out.println("Procent uspešnosti: " + String.format("%.2f%%", rs.getDouble("procent_uspesnosti")));
                System.out.println("Ocenjena zahtevnost: " + rs.getInt("ocenjena_zahtevnost"));
                
                Timestamp lastUsed = rs.getTimestamp("zadnja_uporaba");
                if (lastUsed != null) {
                    System.out.println("Zadnja uporaba: " + new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(lastUsed));
                } else {
                    System.out.println("Zadnja uporaba: Nikoli");
                }
            }
        }
    }
