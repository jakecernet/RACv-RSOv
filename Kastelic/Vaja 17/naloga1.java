/* Vislice_4 predstavlja izvedbo mehanizma ugibanja 'naključno' izbranega gesla. V delujoči demonstraciji geslo izberemo izmed gesel v slovarju, predstavljenim s tabelo nizov. 
Hkrati sta podana še 2 načina pridobivanja podatkov (iz datoteke, iz spletnega vira), v katerih ni dokončno realizirana izbira enega samega gesla. 
Po vzoru rabe vira, predelajte program tako, da bo geslo izbral izmed gesel podatkovne zbirke z enim od izbranih mehanizmov (izberete si ga sami):

Izbrani mehanizem: geslo izbrane dolžine, ki se najdalj časa ni uporabilo */
...
public class naloga1 {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            while (!gameOver && wrongGuesses < 6) {
                ...
                if (isWordGuessed()) {
                    ...
                    updatePasswordSuccess(conn, passwordId, true);
                } else if (wrongGuesses >= 6) {
                    gameOver = true;
                    System.out.println("\n✗ IZGUBIL SI! Geslo je bilo: " + currentPassword);
                    updatePasswordSuccess(conn, passwordId, false);
                }
            }
     ...
     * Implements the mechanism: "geslo izbrane dolžine, ki se najdalj časa ni uporabilo"
     */
    ...
    private void updatePasswordSuccess(Connection conn, int id, boolean success) throws SQLException {
        String sql = """
            UPDATE besede 
            SET zadnja_uporaba = ?, 
